...
export function HostLobby({ game, teacherToken, onExit }: { game: HostedGame; teacherToken: string; onExit: () => void }) {
  ...
  useEffect(() => {
    ...
    socket.connect();
    return () => { socket.off("connect", watchLobby); socket.off("game:lobby-updated", setLobby); socket.off("game:teacher-state", setGameState); socket.off("connect_error"); socket.disconnect(); };
  ...
  return <main className="lobby-screen game-host">
    ...
    {status === "LOBBY" ? <section className="lobby-grid">
      ...
      <div className="game-status"><span>{status.replaceAll("_", " ")}</span>{seconds !== null && <strong>{seconds}s</strong>}<small>{gameState?.playerCount ?? 0} players</small></div>
      ...
      {status === "QUESTION_SHOW" && <p className="teacher-notice">Get ready — answer buttons open in a moment.</p>}
      {status === "ANSWER_COLLECT" && <p className="teacher-notice">Answers are being collected. The countdown is owned by the server.</p>}
      {gameState?.leaderboard && status !== "FINAL_PODIUM" && <section className="teacher-panel leaderboard"><h2>Leaderboard</h2><ol>{gameState.leaderboard.slice(0, 10).map((entry) => <li key={entry.playerId}><span>#{entry.rank} {entry.nickname}</span><strong>{entry.score}</strong></li>)}</ol></section>}
